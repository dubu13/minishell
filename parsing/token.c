...
/*   Updated: 2024/07/01 20:14:47 by dkremer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *
	...
	* or a here-document ('<<'). It creates a new token with the appropriate type and value,
	and adds it to the provided token list.
 *
...
/**
 * Handles the parsing of a meta character (such as '>', '<',
		or '|') in the input string.
 *
 ...
 *
 * This function is responsible for identifying a word token,
	extracting its value, creating a new token,
	and adding it to the provided token list.
 * If a meta character (such as '>', '<', or '|') is encountered after the word,
	the function will also call the `handle_meta_char` function to process the meta character.
 *
 * @param input       The input string being tokenized.
 * @param i           A pointer to the current index in the input string.
 ...
 * This function is responsible for identifying the token type (word, pipe,
		redirection)
 ...
 *
	...
	* This function is responsible for iterating through the input string and identifying the different token types (word,
